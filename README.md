...
| Product            | When to add                                            |
| ------------------ | ------------------------------------------------------ |
| `UniTrack`         | Always — the core SDK (kèm built-in FirebaseAdapter). |
| `UniTrackSnowplow` | To forward events to a Snowplow collector.             |

> `UniTrackSnowplow` pulls in `snowplow-ios-tracker`. Add only nếu cần Snowplow.
> Firebase Analytics dùng built-in adapter (reflection, không cần product riêng).
...
### With Firebase Analytics (built-in adapter)

App tự link Firebase qua CocoaPods hoặc SPM, sau đó:

```swift
import UniTrack

// Reflection-based — UniTrack KHÔNG import Firebase. Tìm class
// FIRAnalytics runtime, no-op nếu Firebase chưa link.
UniTrack.attachFirebaseAdapter()
UniTrack.initialize(apiKey: "YOUR_API_KEY")
```

Adapter stamp `session_id` + `session_index` vào MỌI event Firebase qua
`setDefaultEventParameters` — app gọi `Analytics.logEvent(...)` trực tiếp
bypass UniTrack vẫn có session_id để join cross-provider.
